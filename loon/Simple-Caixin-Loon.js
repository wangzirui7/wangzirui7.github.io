...
(function () {
  ...
  var baseCSS = [
    ...
    '/* 移除正文背景水印（含 App 动态注入的伪元素） */',
    'html, body, #cx-customer, #cx-main, .cx-app-content-main, #cx-cons, .cx-cons,',
    'html::before, html::after, body::before, body::after,',
    '#cx-customer::before, #cx-customer::after, #cx-main::before, #cx-main::after,',
    '.cx-app-content-main::before, .cx-app-content-main::after,',
    '#cx-cons::before, #cx-cons::after, .cx-cons::before, .cx-cons::after { background: none !important; background-image: none !important; }',
    '.watermark, .water-mark, .cx-watermark, .cx-water-mark, .shuiyin, .cx-shuiyin,',
    '[id*="watermark"], [class*="watermark"], [id*="water-mark"], [class*="water-mark"],',
    '[id*="shuiyin"], [class*="shuiyin"], [id*="水印"], [class*="水印"] { display: none !important; }',
  ...
  var pageRuntime = [
    ...
    '  }',
    '  var watermarkSelector = ".watermark, .water-mark, .cx-watermark, .cx-water-mark, .shuiyin, .cx-shuiyin, [id*=\\"watermark\\"], [class*=\\"watermark\\"], [id*=\\"water-mark\\"], [class*=\\"water-mark\\"], [id*=\\"shuiyin\\"], [class*=\\"shuiyin\\"], [id*=\\"水印\\"], [class*=\\"水印\\"]";',
    '  function hideWatermark(node) {',
    '    if (node && node.style) node.style.setProperty("display", "none", "important");',
    '  }',
    '  function suppressWatermarks(root) {',
    '    var scope = root || document;',
    '    if (scope.nodeType === 1 && scope.matches && scope.matches(watermarkSelector)) hideWatermark(scope);',
    '    if (!scope.querySelectorAll) return;',
    '    var nodes = scope.querySelectorAll(watermarkSelector);',
    '    for (var index = 0; index < nodes.length; index += 1) hideWatermark(nodes[index]);',
    '  }',
    ...
    '      clearTimeout(timer);',
    '      timer = setTimeout(function () { suppressWatermarks(document); }, 30);',
    '    });',
    ...
    '  }',
    '  function start() { applyAudioSetting(); suppressWatermarks(document); observeWatermarks(); addAudioButton(); }',
